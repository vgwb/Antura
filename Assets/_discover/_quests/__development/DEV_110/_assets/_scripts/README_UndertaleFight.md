...
| `FightBullet.cs` | One projectile: moves, hit-tests the soul, despawns off-box or on lifetime. |
...
- **DodgeWaves** list: per wave set `Id`, `BulletPrefab` (an Image + `FightBullet`), `BulletsPerSpawn`,
  `SpawnInterval`, `Speed`, `Duration`, `DamagePerHit`, `Shape` (RandomEdges / TopDown / Sides).

Bullet prefab = a small UI `Image` with a `FightBullet` component (RectTransform required).
