...
namespace Antura.Debugging
{
    ...
    public class DebugControlsManager : MonoBehaviour
    {
        ...
        readonly DebugCommand[] _globalCheats = new[] {
            ...
            new DebugCommand("Set timeScale to 10x (while pressed)", CommandCondition.WhilePressed, KeyCode.X, KeyModifier.Shift, on => {
                Time.timeScale = on ? 10 : 1;
            }),
            new DebugCommand("Set timeScale to 0.2x (while pressed)", CommandCondition.WhilePressed, KeyCode.Z, KeyModifier.Shift, on => {
