...
Every quest script starts with a comment block identifying the quest and listing available scene elements. The `<scene_data>` block is **auto-generated by the Unity editory** using the **Quest Management Window**.
...
> **Detailed technical reference:** The [Quest Development Guide](quest-development.md) covers Unity setup, VS Code extensions, folder naming, Addressables, and a full step-by-step scene wiring checklist. This section is a summary for designers who need to communicate with developers.
...
For the full rule set, examples, and how to use AI to review the lines of a script, see [Quest Script Writing Guidelines](quest-scripts-guidelines.md).
